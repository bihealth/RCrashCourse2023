...
`labresults_full.csv` contain data from the same study.
 ...
 1. Which columns describe the ID of the subjects? Are there any subjects in common? How do
    you match the subjects?
 1. We are interested only in the following information: Subject ID, ARM
    (group), Time point, sex, age, test name and the actual measurement.
    Are the measurements numeric? Remember, you can use expressions like `[ , c("ARM", "sex") ]` or `select()` to select the desired columns from a data set.
 1. Use the subjects to merge the two data frames however you see fit. Note
    that there are multiple time points per subject and multiple
    measurements per subject and time point.
    
## Exercise 5.1 - Explorative analysis

After merging your data, we can now start with an exploratory analysis of this data. In particular, we are interested in the concentration of neutrophils over time and treatment.

 1. Is your data frame consistent?
 1. How can you filter the labresults for neutrophils?
 1. Create a data frame in wide format with columns denoting the measurements over time?
 1. Plot the distribution of neutrophils over time.
 1. What kind of descriptive analysis would you do to get familiar with this data set?
